...
- **Rationale:** Zero setup for the evaluator — no Docker, no connection strings, no environment variables. DB file lives at `server/dev.db` and is created on first run.
...
- **Reversible?** Partially — schema is simple enough to port, but it's a non-trivial migration step.
...
| Question | Owner | Due |
|----------|-------|-----|
| Should a PATCH with multiple changes be atomic — if one change fails, do all roll back? Or apply up to the failure and return partial results? | Candidate | Before building PATCH |
